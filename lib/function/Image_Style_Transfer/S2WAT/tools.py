...
@torch.no_grad()
def save_transfer_imgs(network, content_dir: str, style_dir: str, output_dir: str, device=torch.device('cpu')):
    ...
    for c_name in tqdm(c_names, desc='Content images'):
        c_path = os.path.join(content_dir, c_name)

        if not os.path.isfile(c_path):
            continue

        c_tensor, _ = content_style_transTo_pt(c_path, c_path)  # 第二个参数占位，无用
        for s_name in tqdm(s_names, desc='Style images', leave=False):
            ...
            out_name = f'{stem_c}_+_{stem_s}{ext_c}'
